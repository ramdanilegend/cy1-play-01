...
const RowsDailyCalled = (props) => {
  ...
  const getDropdown = async (data) => {
    ...
    if (!open) {
      try {
        const response = await AnalyticService.postDropdownCalled({
          BNumber: data.called,
          daterangepicker: dashboardContext.state.dashboardView.daterangepicker,
        });
        setDataDropdown(response.data.tabledatahits);
        setTimeout(() => {
          setError(false);
        }, [500]);
      } catch (error) {
...
export default RowsDailyCalled;
